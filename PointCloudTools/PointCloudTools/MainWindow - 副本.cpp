...
void MainWindow::openRun()
{
	...
	for (int i = 0; i != m_strFileNames.size(); i++)
	{
		...
		if (strFileName.endsWith(".pcd", Qt::CaseInsensitive))
		{
			...
			//}
			status = pcl::io::loadPCDFile(file_name, *m_pointCloudFileTI);
		}
		...
		indices.clear();
 		pcl::removeNaNFromPointCloud(*m_pointCloudFileTI, *m_pointCloudFileTI, indices);
		//22.6519171283,114.04355726,75.3121337891,-64.9127807617
		NavStaFix inputNav;
		inputNav.dLatitude = 22.6519171283;
		inputNav.dLongitude = 114.04355726;
		inputNav.dAltitude = 75.3121337891;

		const double angle = 64.9127807617;
		//e2 = 2 * f - f * f;  
		//ee = e2 * (1.0 - e2);
		double e1, e2;
		...
		//第二偏心率的平方
		e1 = pow(sqrt(a * a - b * b) / a, 2);
		//第二偏心率的平方
		e2 = pow(sqrt(a * a - b * b) / b, 2);

		ofstream outfile("parameter.txt", ios::trunc);
		outfile.setf(ios::fixed, ios::floatfield);
		outfile.precision(12);

		//获取原点在高斯坐标系下的值
		double dOriginX = 0.0, dOriginY = 0.0;
		BLH2xyz(22.6519171283, 114.04355726, a, e1, e2, -1, dOriginX, dOriginY);
		outfile << "Origin Longitude:" << inputNav.dLatitude << " Origin Latitude:" << inputNav.dLatitude << " Origin Altitude:" << inputNav.dAltitude << "\n";
		...
		for (size_t i = 0; i < m_pointCloudFileTI->size(); ++i)
		{
			...
			pta.outNav = getNavStaFixByPointxyz(m_pointCloudFileTI->points[i], inputNav, angle);
			//outfile << pta.outNav.dLongitude << "," << pta.outNav.dLatitude << "," << inputNav.dAltitude << "\n";
			double dX = 0.0, dY = 0.0;
			BLH2xyz(pta.outNav.dLatitude, pta.outNav.dLongitude, a, e1, e2, -1, dX, dY);
			//减一个统一的坐标原点
			m_pointCloudFileTI->points[i].x = static_cast<float>(dX - dOriginXTemp);			
			m_pointCloudFileTI->points[i].z = static_cast<float>(pta.outNav.dAltitude + m_pointCloudFileTI->points[i].y);
			m_pointCloudFileTI->points[i].y = static_cast<float>(dY - dOriginYTemp);
...
void MainWindow::BLH2xyz(double dLat, double dLon, double a, double e1, double e2, int k, double &x, double &y)
{
	//公式精确到0.001米
...
void MainWindow::itemSelectedData(QTreeWidgetItem* item, int count)
{
	...
	//原点经纬度、海拔高度、方位角
	NavStaFix inputNav;
	inputNav.dLatitude = 22.6571520000;
	inputNav.dLongitude = 114.0562170000;
	inputNav.dAltitude = 68.2548;
	...
	for (size_t i = 0; i < m_pointCloudFile.cloud->size(); ++i)
	{
		//PointAttribute pta;
		//pta.iIndex = i;
		//pta.pt = m_pointCloudFile.cloud->points[i];

		//pta.outNav = getNavStaFixByPointxyz(m_pointCloudFile.cloud->points[i], inputNav);
		//m_pointCloudFile.cloud->points[i].x = pta.outNav.dLongitude;
		//m_pointCloudFile.cloud->points[i].y = pta.outNav.dLatitude;
		//m_pointCloudFile.cloud->points[i].z = pta.outNav.dAltitude;
		//m_pointAttributeVec.push_back(pta);
	}
