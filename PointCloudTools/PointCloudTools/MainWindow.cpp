...
void MainWindow::openRun()
{
	...
	for (int i = 0; i != m_strFileNames.size(); i++)
	{
		...
		//std::string ddd = "E:\\VTK\\pcd\\20191211\\A2.pcd";
		//更新状态栏
		...
		long dOriginXTemp = static_cast<long>(dOriginX);
		long dOriginYTemp = static_cast<long>(dOriginY);
...
void MainWindow::conv_llh2xyz(double latitude, double longitude, double &Y, double &X)
{
	double PS;   //
	double PSo;  //
	double PDL;  //
	...
	double PA, PB, PC, PD, PE, PF, PG, PH, PJ;
	double Pe;   //
	double Pet;  //
	double Pnn;  //
	double AW, FW, Pmo;
	...
	PJ = (double)765765.0 / 7516192768.0 * pow(Pe, 16);
	...
	PB9 = (double)AW * (1.0 - pow(Pe, 2)) * PJ / 16.0;

	PS = (double)PB1 * latitude + PB2 * sin(2.0 * latitude) + PB3 * sin(4.0 * latitude) + PB4 * sin(6.0 * latitude) +
		PB5 * sin(8.0 * latitude) + PB6 * sin(10.0 * latitude) + PB7 * sin(12.0 * latitude) + PB8 * sin(14.0 * latitude) +
		PB9 * sin(16.0 * latitude);
	...
	PDL = (double)longitude - m_PLo;
	Pt = (double)tan(latitude);
	PW = (double)sqrt(1.0 - pow(Pe, 2) * pow(sin(latitude), 2));
	PN = (double)AW / PW;
	Pnn = (double)sqrt(pow(Pet, 2) * pow(cos(latitude), 2));

	X = (double)((PS - PSo) + (1.0 / 2.0) * PN * pow(cos(latitude), 2.0) * Pt * pow(PDL, 2.0) +
		(1.0 / 24.0) * PN * pow(cos(latitude), 4) * Pt *
		(5.0 - pow(Pt, 2) + 9.0 * pow(Pnn, 2) + 4.0 * pow(Pnn, 4)) * pow(PDL, 4) -
		(1.0 / 720.0) * PN * pow(cos(latitude), 6) * Pt *
		(-61.0 + 58.0 * pow(Pt, 2) - pow(Pt, 4) - 270.0 * pow(Pnn, 2) + 330.0 * pow(Pt, 2) * pow(Pnn, 2)) *
		pow(PDL, 6) -
		(1.0 / 40320.0) * PN * pow(cos(latitude), 8) * Pt *
		(-1385.0 + 3111 * pow(Pt, 2) - 543 * pow(Pt, 4) + pow(Pt, 6)) * pow(PDL, 8)) *
		Pmo;

	Y = (double)(PN * cos(latitude) * PDL -
		1.0 / 6.0 * PN * pow(cos(latitude), 3) * (-1 + pow(Pt, 2) - pow(Pnn, 2)) * pow(PDL, 3) -
		1.0 / 120.0 * PN * pow(cos(latitude), 5) *
		(-5.0 + 18.0 * pow(Pt, 2) - pow(Pt, 4) - 14.0 * pow(Pnn, 2) + 58.0 * pow(Pt, 2) * pow(Pnn, 2)) *
		pow(PDL, 5) -
		1.0 / 5040.0 * PN * pow(cos(latitude), 7) *
